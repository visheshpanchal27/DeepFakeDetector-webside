...
class DeepFakeDetector:
    ...
    def _detect_ai_text_watermark(self, image):
        ...
        try:
            ...
            for region in regions:
                ...
                # Check for uniform brightness areas (watermark background)
                region_std = np.std(region)
                region_mean = np.mean(region)
                if region_std < 15 and (region_mean < 50 or region_mean > 200):
                    if edge_ratio > 0.02:
                        return True
    ...
    def _predict_image(self, image):
        ...
        try:
            ...
            if edge_density < 0.08 or edge_density > 0.25:
                ai_indicators += 1
